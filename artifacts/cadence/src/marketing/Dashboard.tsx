...
export function Dashboard() {
  const { campaigns, tasks } = useAppState();
  const rollup = {
    todo: tasks.filter((t) => t.status === "backlog").length,
    inProgress: tasks.filter((t) => t.status === "in_progress").length,
    inReview: tasks.filter((t) => t.status === "in_review").length,
    done: tasks.filter((t) => t.status === "done").length,
    overdue: tasks.filter((t) => t.dueDate === "Yesterday" && t.status !== "done").length,
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="rounded-xl p-3 bg-[var(--c-surface-2)] border border-[var(--c-border)]">
                  ...
                  <div className="text-[20px] font-display font-bold">{rollup.todo}</div>
                </div>
                <div className="rounded-xl p-3 bg-[var(--c-surface-2)] border border-[var(--c-border)]">
                  <div className="text-[12px] font-medium text-[var(--c-ink-soft)] mb-1">In Progress</div>
                  <div className="text-[20px] font-display font-bold">{rollup.inProgress}</div>
                </div>
                <div className="rounded-xl p-3 bg-[var(--c-surface-2)] border border-[var(--c-border)]">
                  <div className="text-[12px] font-medium text-[var(--c-ink-soft)] mb-1">In Review</div>
                  <div className="text-[20px] font-display font-bold">{rollup.inReview}</div>
                </div>
                <div className="rounded-xl p-3 bg-emerald-50/50 border border-emerald-100">
                  ...
                  <div className="text-[20px] font-display font-bold text-emerald-700">{rollup.done}</div>
                </div>
                <div className="rounded-xl p-3 bg-red-50/50 border border-red-100">
                  ...
                  <div className="text-[20px] font-display font-bold text-red-700">{rollup.overdue}</div>
                </div>
                <div className="rounded-xl p-3 bg-amber-50/50 border border-amber-100">
                  ...
                  <div className="text-[20px] font-display font-bold text-amber-700">{rollup.blocked}</div>
                </div>
              </div>
